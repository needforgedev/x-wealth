...
- [~] **W7-01** Read-only analysis service. **Never writes to a strategy definition** — a legal boundary, not a preference (§10.6). **Post-mortem half done 25 Sep 2026** — `src/domain/post-mortem.ts` + `src/server/actions/post-mortem.ts`, which imports nothing that can write: no strategy actions, no `@/db`, and its only database statement is the interaction log itself. The CRITIQUE context (backtest half, W7-03…07) remains
...
- [~] **W7-11** Critique view UI — **post-mortem section done 25 Sep 2026** on `/forward-tests/[id]` for COMPLETED windows, above the curve for W18-09's reason: the hypothesis line qualifies every figure below it. The backtest critique view remains with W7-03…07
