...
| **AD-12** | Test stack | ⭐ Vitest + Playwright (Playwright already used ad hoc) | ☐ Open |
...
- [~] **W1-02** Drizzle + `postgres` + `@supabase/supabase-js` installed; `drizzle.config.ts`, `src/db/`, `drizzle/` and `.env.example` committed; `db:generate` / `db:migrate` / `db:studio` / `db:check` scripts added. **Remaining:** create the Supabase project, fill `.env.local`, run the migration
...
- [~] **W1-05** `drizzle/verify_invariants.sql` written — 21 assertions, transactional, leaves no residue. **Verified green against an embedded Postgres, and negative-controlled** by dropping each trigger and confirming it catches the breach. **Remaining:** run it against the real Supabase project, twice, once as `service_role`
- [~] **W1-06** `assert_no_soft_delete_columns()` written and tested. Implemented as a callable assertion rather than a DDL event trigger because **`CREATE EVENT TRIGGER` needs superuser and Supabase's `postgres` role is not one**. **Remaining:** wire into CI (W1-15)
- [ ] **W1-07** Supabase Auth: phone OTP + SMS provider (MSG91), sessions, role split (advisor / investor / admin) via `auth.users` FK + a role claim
- [ ] **W1-08** Wire the built OTP screens to real auth (`/otp`, `/advisor/otp`, `/alpha/otp`, `/alpha/verify-number`)
- [ ] **W1-09** Middleware-level registration gate (spec §5.4) — **not per-endpoint**
...
- [ ] **W1-12** Money/price helpers with the float trap tested (paise integers, decimal prices)
- [ ] **W1-13** IST session calendar + exchange holiday calendar, with date arithmetic tests
- [ ] **W1-14** Symbol model with exchange qualification (`NSE:RELIANCE` ≠ `BSE:RELIANCE`)
- [ ] **W1-15** CI: typecheck, lint, test on every push
- [ ] **W1-16** Admin panel shell (role-gated, audit-logged)
- [ ] **W1-17** RLS policies as defence in depth on every user-facing table — never as the sole enforcement of an invariant
