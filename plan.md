...
- [x] **W1-08** Both paths wired and driven end to end in a browser. **Investor:** `/` → `/otp` → `/complete-profile` → `/onboarding-questions` → `/choose-interests` → `/risk-disclosure` → `/investor/home`. **Advisor:** `/` → `/advisor/otp` → … → `/ops` → verified. Destinations computed from account state on both sides; every page server-guarded. **AD-15 resolved: the Investor screens are canonical; `/alpha/*` left untouched.** Old note follows — Advisor path wired and driven end to end in a browser: `/` → `/advisor/otp` → `/advisor/complete-profile` → `/advisor/kyc` → `/advisor/status` → `/ops` → verified. Sign-in destination is computed from the account's state (`src/domain/advisor-onboarding.ts`, 12 tests), and every advisor page is guarded server-side. Investor and Alpha OTP screens still navigate-only, pending AD-15 (PRD Phase 1 is advisor-only, so they are not on the critical path)
...
- [x] **W11-01** Investor onboarding wired to real auth: profile, experience level, interests, all persisted. `src/domain/investor-onboarding.ts` decides the step (7 tests)
- [x] **W11-02** Mandatory risk-disclosure acknowledgement + suitability capture, at `/risk-disclosure`. Three points acknowledged separately, not one blanket agreement; recorded with a timestamp and a `suitability` snapshot. `nextInvestorStep` refuses to let anyone past it — verified by attempting to deep-link `/investor/home` before acknowledging
...
| 19 Aug 2026 | **Investor flow wired (W11-01, W11-02); AD-15 resolved in favour of the Investor screens.** Migration `0005` adds investor contact fields. Signup → profile → experience → interests → **mandatory risk acknowledgement** → home, driven end to end and confirmed in Postgres. The risk gate is enforced, not decorative: deep-linking `/investor/home` before acknowledging redirects back. `/investor/home` shows an honest empty state — no strategy can be discoverable until an advisor completes a forward test, and inventing sample groups is the exact thing this product exists to stop. 111 tests. |
