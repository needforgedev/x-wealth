...
- [x] **W1-13** **Done 22 Sep 2026.** `src/domain/session.ts` — IST sessions, timezone-independent. `PLACEHOLDER_CALENDAR_2026` is replaced by `NSE_CALENDAR`: 71 holidays across 2023–2026, sourced from the exchange circular as republished by Zerodha, Groww and CalendarLabs and cross-checked per year. The placeholder carried **three holidays for one year**, and nothing ever failed because of it — a 60-session window opened 2 March 2026 simply reported `planned_end_at` of 22 May when the sixtieth session falls on 2 June, seven sessions later. That sat inside `check-forward-tests`' 21-day overrun tolerance, so it raised no alarm; it consumed **twelve of the twenty-one days** the alarm had to detect a genuinely stuck test. **The trap on the way in was Diwali Laxmi Pujan**: a gazetted trading holiday on which the exchange nonetheless runs the Muhurat session, so prices print and a bar exists. Listing it as a holiday and stopping there would have made `assertValidSeries` reject four genuine bars — the same failure the six weekend sessions caused on the first backfill. Each is listed in `holidays` *and* `specialSessions`, which wins, so the pair records both facts rather than one. `NSE_CALENDAR_COVERS` is exported because the gap is the remaining hazard: outside 2023–2026 every holiday still reads as a trading day
