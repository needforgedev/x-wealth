...
- [~] **W6-15** **New, and urgent for the calendar.** Start one real, locked, 60-session forward test **now**, while the rest is built. The engine works. This is the single scheduling decision that determines whether N6 lands in ~7 months or ~10. **25 Sep 2026 — the pipeline half is proven, the calendar half is not.** A first real test is RUNNING (TCS v1, 20 sessions, opened 23 Sep, planned end 22 Oct, engine pinned by `W6-17`) and the evening workflow is green end to end after two fixes: the lockfile was missing the wasm optional-dep edges npm 10 validates and npm 11 does not — regenerated with `npx npm@10`, and `npx npm@10 ci --dry-run` is now the pre-push check for any dependency change — and the Actions secrets had gone stale over the September outage. **Still open:** this is a 20-session shakedown of a strategy whose backtest produced zero trades (RSI(14) < 14 almost never prints), so it exercises the machinery, not the milestone — the 60-session window this item exists for has not started, and each day it waits is a day on N6
...
- [x] **W7-01** **Done 25 Sep 2026, both halves.** Read-only analysis as a legal boundary (§10.6): `post-mortem.ts`/`critique.ts` in the domain, one action each, and neither action imports anything that can write — no strategy actions, no `@/db`, the interaction log as the only database statement, pinned by source-scan tests (W7-10). The two contexts share one no-verdict vocabulary (`ai-gate.ts`), because a graded word refused on one surface and rendered on another is a gate in name only. Migration `0016` adds the CHECK `0013` deliberately deferred — a CRITIQUE names its strategy version — now that W7's shape is a fact rather than a guess
- [x] **W7-02** **Done 25 Sep 2026, as a runtime gate rather than a test.** The adversarial suite holds this line in its tests (W18-10) because its producer is our own deterministic code; the post-mortem's producer is a model, so the same bar — findings carry digits, no judgement vocabulary, no key anywhere named score/grade/rating/rank/stars/verdict/quality — runs in `validatePostMortem` on every output, stored ones included: the console re-gates on every read, so a narrative the current gate refuses disappears rather than surviving as a grandfathered verdict
- [x] **W7-03** **Done 25 Sep 2026, by reuse.** The parameter list the critique reads is `tunableParameters()` — the same enumeration the sensitivity sweep perturbs (W18-02), so the critique and the sweep can never disagree about what counts as a parameter. Count and values are computed and stated to the model; the model observes, never derives
- [x] **W7-04** **Done 25 Sep 2026.** `sampleAdequate` is read from the recorded metrics (§8.12, computed by the engine), stated to the model, and the prompt requires an inadequate sample to appear in the findings — absent on old rows reads as inadequate, never as fine
- [x] **W7-05** **Done 25 Sep 2026, via the attack report.** Regime dependence is W18-03's computed finding; the critique passes the report through and narrates it. Without a report, the critique's `limits` must say the question is open — required field, because silence has to be legible (§7.13's rule, applied to unanswerable questions)
- [~] **W7-06** Liquidity feasibility at stated size — **the slippage half is done** (cost-sensitivity break-even from W18-05 flows through the attack report; the cost basis rides in every snapshot). **Still open:** a computed turnover-versus-position-size signal, which no engine module produces yet — the critique can only observe what is recorded, and this is not recorded
- [x] **W7-07** **Done 25 Sep 2026.** Max drawdown, longest losing streak, best trade's share and the Monte Carlo drawdown distribution (via the report) are all in the snapshot; the first live run's findings led with exactly these
- [-] **W7-08** ~~Plain-language explanation *for investors*~~ — no investors. **Replaced by W7-12**
- [x] **W7-09** **Landed with W15-03 and exercised here 25 Sep 2026** — the post-mortem reaches the model only through `runInteraction`, so the snapshot, output, model id and prompt version are recorded before anything renders, and the recorded row *is* what the console reads back
- [x] **W7-10** **Done 25 Sep 2026.** Two locks in `post-mortem.test.ts`: the domain module's export list is pinned, and the action's source is scanned for `createStrategy`, `reviseStrategy`, `from "@/db"` and raw `.insert(`/`.update(`/`.delete(` — the guarantee is about the whole path from model output to database, not one module's manners
- [x] **W7-11** **Done 25 Sep 2026, both views.** The post-mortem section on `/forward-tests/[id]` (above the curve — the hypothesis line qualifies every figure below it, W18-09's reason) and the critique section on `/backtests/[id]`, directly under the metrics it bounds: the attack report asks whether the numbers survive attack, the critique asks how much the record can carry in the first place. Both re-gate the stored output on every read, render withheld-finding counts out loud, and carry the same closing line: it describes, it grades nothing, deciding is the author's
- [x] **W7-12** **Done 25 Sep 2026 for the post-mortem** — the required `summary` field, gated like everything else, written to the author about their own window
- [x] **W7-13** **Done 25 Sep 2026 — the post-mortem, proven against a live model.** `POST_MORTEM` joins the registry with a strict schema whose only status values are SUPPORTED, NOT_SUPPORTED and UNTESTED — a statement about *the hypothesis against the data*, which is the falsifiability the loop exists for, never about quality, which §8.7 forbids anyone to grade. **The zero-trade rule is ours, not the model's**: `tradeCount === 0` forces UNTESTED deterministically, because a window the rules never traded in answered nothing in either direction — and the first real window (TCS, 20 sessions) is exactly this case. **Pressing again returns the record** (W18-12's shape): the first live, valid answer under the current prompt version is what every later press returns, because two narratives of one fixed record invite choosing the kinder one. **The gate fired on its first live outing** — seven findings came back and one carried no number — which forced the blast-radius decision: a numberless finding is *withheld and counted on screen* ("N observations withheld for carrying no figures"), while judgement vocabulary anywhere still fails the whole answer, withheld findings included. `npm run verify-post-mortem` seeds a 120-session window inside a rolled-back transaction, completes it with the real engine, and runs the real model: NOT_SUPPORTED on a 50% hit rate against a "more winners than losers" hypothesis, four evidenced findings including one nobody scripted — both trades on RELIANCE, none on TCS. ABANDONED tests are refused on purpose: the reason recorded at abandonment is the explanation, written at the time (W17's principle). 422 tests
...
- [x] **W15-04** **Done 25 Sep 2026.** `src/domain/hypothesis.ts`: the deliverable is a sentence *pair* — the expectation and `wouldBeWrongIf`, its negation — plus `horizonSessions`, bounded to `SESSION_WINDOW` so the workbench can never bless a hypothesis no forward test could run. The gate refuses a falsification condition that restates the expectation, and refuses a sharpened answer with zero challenges: unchallenged is unexamined. Challenges must arrive as questions about the mechanism — the judgement-vocabulary ban forces the phrasing. Too vague to negate → `NEEDS_INPUT` with questions, the compiler's refusal-to-guess applied to premises
- [x] **W15-05** **Done 25 Sep 2026, deliberately modest.** `priorArt` is up to three one-line idea families from the model's general knowledge — no citations, no figures, and the screen says so: a model with no web access asked for references invents them, so the honest shape is a pointer for the trader's own reading, never a fact
- [x] **W15-06** **Done 25 Sep 2026, three locks rather than one prompt.** The input type carries only the trader's words (no field a bar, catalogue or metric could arrive in — pinned by a test on the input's key set); the action imports *nothing* — no market data, no catalogue, no `@/db`, pinned by source scan; and the prompt forbids invented statistics. The workbench is the one AI surface whose value depends on what it has not seen
- [x] **W15-07** **Done 25 Sep 2026.** The anchor record is the `ai_interactions` row itself — timestamped, append-only, input and output verbatim, `user_acted` set when the trader adopts the result (`markHypothesisUsed`, no `resulting_version_id` — nothing is authored yet). The 0013 CHECK holds the §7.2 line: a HYPOTHESIS row anchored to an existing version is refused by the database as the rationalisation it would be. The statement floor matches `startForwardTest`'s 30-character minimum, so a blessed hypothesis is always one the declaration form will accept. **The carry is manual by design** — the trader copies the pair into the declaration; the workbench proposes, the declaration freezes
- [x] **W15-08** **Done 25 Sep 2026.** `/workbench`, linked from home above the strategy list because that is its place in the order of work. idea → questions → the pair, with the challenges placed *between* the sharpened statement and the copy button: whoever adopts the hypothesis has at least scrolled past the questions it has to survive. **Live-proof asterisk, same as W7's critique:** `npm run verify-hypothesis` (sharp idea → SHARPENED, vague idea → NEEDS_INPUT) is written and gated, but the free tier spent the afternoon overloaded across models — "Upstream error from Nvidia: Service temporarily overloaded", qwen rate-limited — so no live pass is on record yet. A tiny payload failing with a named overload also retired the payload-size theory from earlier today: it is congestion, not size. 443 tests
...
| Date | Change |
|---|---|
| **25 Sep 2026** | **The hypothesis workbench (W15-04…08 — N1 is code-complete).** Step 1 of the loop, and the one AI surface whose value depends on what it has *not* seen: the input type has no field a bar or a metric could arrive in, the action imports nothing that reads data, and both are pinned by tests (W15-06's three locks). The deliverable is a sentence pair — the expectation and `wouldBeWrongIf`, its negation — plus a horizon bounded to `SESSION_WINDOW`, so the workbench cannot bless a hypothesis no forward test could run; a falsification condition that restates the expectation is refused, and so is a sharpened answer with no challenges. Prior art is deliberately modest: idea families from general knowledge, no citations, because a model without web access asked for references invents them. The anchor record is the interaction row itself, unanchorable to any existing version by the 0013 CHECK. `/workbench` places the challenges between the statement and the copy button on purpose. Live proof (`npm run verify-hypothesis`) is written but unwitnessed — the free tier was overloaded across models all afternoon, which also retired the payload-size theory from the critique's chase: tiny payloads fail the same way, it is congestion. With this, every N1–N5 module has working code; what N6 still needs is the calendar (W6-15's 60-session window) and the pending live proofs. 443 tests. |
| **25 Sep 2026** | **The backtest critique (W7-01/03/04/05/07/11 closed; W7-06 half), and the gate catching a real model red-handed.** `CRITIQUE` joins the registry: facts assembled from the run's recorded results, the parameter list reused from `tunableParameters()` so the critique and the sensitivity sweep can never disagree about what counts as a parameter, and the attack report passed through where one exists — critique without one is allowed and must name what it cannot know under `limits`, a required field. Migration `0016` adds the CHECK that `0013` deliberately deferred: a critique names its strategy version. One vocabulary now guards both narrating contexts (`ai-gate.ts`). **The most instructive moment was a failure**: with the default model congested, OpenRouter's free meta-router handed the full payload to a 2.6B model, which wrote a graded summary — and `validateCritique` refused it for judgement vocabulary. The negative control, fired by a model nobody scripted, under production conditions. **The asterisk, stated plainly**: no full-payload *pass* has been witnessed yet — five attempts across three free models hit empty 200-bodies and upstream 429s on Friday-evening capacity, while tiny and attack-bearing probes through the same code passed the gate live. `npm run verify-critique` is the standing proof and passes when the tier does. Three provider hardenings came out of the chase, each a production defect in waiting: a 120s per-attempt timeout (a queued call held a connection open past six minutes, which is a hung server action in front of a user), staggered retry backoff (three immediate retries re-ask the same dying upstream), and envelope errors that quote the raw body instead of the word "unparseable". 431 tests. |
| **25 Sep 2026** | **The post-mortem, live (W7-13; W7-02/09/10/12 with it).** Step 6 of the loop existed only as prose until today. Now: `POST_MORTEM` in the provider registry under a strict schema, facts assembled entirely from recorded rows — frozen rules in the console's own words, `final_results` quoted as written, the trades, the cost basis, and the latest backtest of the same version for the forward-versus-backtest comparison — with the model connecting numbers to the declared hypothesis and nothing else. Three design points worth recording. **The status enum is about the hypothesis, not the strategy**: SUPPORTED / NOT_SUPPORTED / UNTESTED is falsifiability doing its job, and the zero-trade case is decided by our code, not the model's judgement. **§8.7 is a runtime gate here, not a test**, because the producer is a model: digits required per finding, judgement vocabulary fatal anywhere, banned key names walked recursively, and stored outputs re-gated on every read. **The gate's first live outing found the design error**: one numberless finding out of seven failed the whole answer, so withholding became per-finding — counted on screen as withheld, never silently dropped — while tone stays fatal for the answer entire. Proven by `npm run verify-post-mortem` against the live model on a real 120-session window inside a rolled-back transaction. The backtest-critique half of W7 (W7-03…07) stays open. 422 tests. |
