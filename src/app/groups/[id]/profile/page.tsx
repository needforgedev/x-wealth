...
import { TAG_TONES, type GroupTag } from "@/lib/groups";
import { RECENT_SIGNALS } from "@/lib/signals";
import { GROUP_PROFILE, PLANS } from "@/lib/subscription";
...
export default function GroupProfilePage({ params }: PageProps<"/groups/[id]/profile">) {
  const { id } = use(params);
  ...
  return (
    <AppShell className="bg-surface-alt">
      <TopBar showBack backHref={`/groups/${id}`} />
      ...
      <SectionHeader
        className="mt-[24px] shrink-0 px-[26px]"
        icon={{ src: "/assets/icon-credit-card.svg", width: 17.5, height: 14 }}
        title="Subscribe Now"
      />

      <section className="mx-[29px] mt-[15px] mb-[29px] bg-surface px-[17px] pt-[16px] pb-[18px]">
        <RadioCardGroup
          name="plan"
        ...
        <button
          type="button"
          onClick={() => router.push(`/groups/${id}/payment`)}
          className="mt-[19px] h-[42px] w-full rounded-[2px] bg-brand text-[14px] font-bold uppercase text-white"
        >
          Buy Now
        </button>
      </section>
