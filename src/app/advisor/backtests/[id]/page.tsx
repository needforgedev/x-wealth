...
import { loadRunForAdvisor } from "@/server/queries/backtest";
 ...
 */
export default async function BacktestRunPage({ params }: PageProps<"/advisor/backtests/[id]">) {
  ...
  const row = await loadRunForAdvisor(id, identity.advisor.id);
  if (!row) notFound();
  ...
  return (
    <AppShell>
      <AppBar backHref={`/advisor/strategies/${row.strategyId}`} />
      ...
      <div className="flex flex-1 flex-col px-5 pb-[calc(29px+env(safe-area-inset-bottom))]">
        ...
        <p className="mt-8 rounded-[6px] bg-surface-alt p-4 text-[13px] text-muted">
          ...
          <Link href={`/advisor/strategies/${row.strategyId}`} className="font-semibold text-brand">
