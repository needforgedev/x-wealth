...
import { loadForwardTestForAdvisor, tradesForForwardTest } from "@/server/queries/forward-test";
...
export default async function ForwardTestConsolePage({
  params,
}: PageProps<"/advisor/forward-tests/[id]">) {
  ...
  const row = await loadForwardTestForAdvisor(id, identity.advisor.id);
  if (!row) notFound();
